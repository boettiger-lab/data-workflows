...
- **HOLD** · `other (no upstream license)` — public-ca30x30/ca-climate-zones  · Kling / Ackerly Lab (UC Berkeley) climate zones from BCM normals, source [github.com/matthewkling/climclust](https://github.com/matthewkling/climclust). **The repository carries no LICENSE file** (`GET /repos/matthewkling/climclust/license` → 404), so no redistribution grant exists and default copyright applies. Excluded via `EXCLUDES[ca30x30]` until Matthew Kling / the Ackerly Lab confirm terms in writing. Not a licensing *problem* — just an unasked question; the data is fine to keep serving on NRP.
