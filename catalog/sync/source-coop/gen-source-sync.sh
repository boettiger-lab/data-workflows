...
REPOS=(
  ...
  social-vulnerability storms trails usfws usgs usgs-nhd usgs-ungulate-migration usgs-wbd utah wetlands
)
            ...
            - name: sync
              ...
              args:
                - |
                  ...
                  # Phase 2 (#158): the data sync above makes source.coop an exact
                  # copy of NRP, clobbering the STAC back to NRP-pointing hrefs — so
                  # re-apply the idempotent rewrite right after. Fetch the committed
                  # script from main (public repo); scope comes from the mounted ConfigMap.
                  echo "=== rewriting STAC hrefs on source.coop (phase 2) ==="
                  RW_URL="https://raw.githubusercontent.com/boettiger-lab/data-workflows/main/catalog/sync/source-coop/rewrite-stac-hrefs.py"
                  if curl -fsSL "$RW_URL" -o /tmp/rewrite-stac-hrefs.py ; then
                    rwflags="" ; [ "${DRYRUN:-false}" = "true" ] && rwflags="--dry-run"
                    SCOPE_FILE=/config/repos.txt python3 /tmp/rewrite-stac-hrefs.py $rwflags \
