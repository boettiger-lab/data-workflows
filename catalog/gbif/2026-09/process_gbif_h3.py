"""
GBIF 2026-09 — Stage 1: H3 index + h0 partition (port of catalog/gbif/2026-06/process_gbif_h3.py).

Reads the 2026-09-01 GBIF occurrence snapshot from AWS Open Data (anonymous),
adds H3 index columns h0..h10 from decimallat/long, filters invalid coords, and
writes h0-partitioned chunks to s3://public-gbif/2026-09/chunks/h0=<int>/.

Changes vs the 2026-06 script (data-workflows#662):
- date 2026-06-01 -> 2026-09-01; output prefix 2026-06 -> 2026-09.
- FILES_PER_CHUNK 42 -> 50. The source grew 8,375 -> 9,898 files; 50*200 >= 9,898
  keeps Stage 1 at 200 completions. Left at 42 the job runs clean and silently
  drops the last ~1,500 source files, so re-check this on every snapshot bump.

Carried over from 2026-06, do not regress:
...
- Keeps h0..h10; the live release carries all 11 resolutions.
"""
...
SRC_PREFIX = "occurrence/2026-09-01/occurrence.parquet/"
OUTPUT = "s3://public-gbif/2026-09/chunks"
FILES_PER_CHUNK = 50
