...
HEX_NOTE = (
    ...
    "**Every per-segment column is repeated in full on every cell the segment touches**, so "
    "`_cng_fid` is the dedup key for all of them. Measured inflation if you SUM raw hex rows "
    "instead: SEG_LENGTH 7.46x (2,710,042 mi against a true 363,358), GIS_MILES 7.43x, "
    "SHAPE_LEN 7.46x, EMP 6.97x, BMP 5.76x. Deduplicate first:\n\n"
    "```sql\n"
    ...
    "-- wrong: SUM(SEG_LENGTH) over raw rows returns 2,710,042 mi — 7.46x the real network\n"
    "```\n\n"
