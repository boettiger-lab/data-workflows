...
- **Backup cadence:** a weekly **`source-sync` CronJob** (Sundays 08:00 UTC, `catalog/sync/k8s/source-sync-cron.yaml`) keeps every in-scope repo current automatically. It loops the generated `source-sync-scope` ConfigMap (`repos.txt`) sequentially at 50 MB/s, continue-on-error. Scope lives in that ConfigMap (regenerated by `gen-source-sync.sh`), so after a scope change run `./gen-source-sync.sh` and `kubectl apply -f catalog/sync/k8s/source-sync-cron-config.yaml`. The per-repo `source-sync-<repo>.yaml` jobs + `run-source-sync.sh` remain for manual/backfill runs.
